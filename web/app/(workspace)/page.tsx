...
export default function HomePage() {
  ...
  const speechBaseRef = useRef(""); // input without any interim speech text
  ...
  const handleSpeechInterim = useCallback((text: string) => {
    setInput(speechBaseRef.current + text);
  }, []);

  const handleSpeechFinal = useCallback((text: string) => {
    const trimmed = speechBaseRef.current.trimEnd();
    const newValue = trimmed ? trimmed + " " + text : text;
    speechBaseRef.current = newValue;
    setInput(newValue);
  }, []);
      ...
      <div className="mx-auto flex w-full max-w-[960px] flex-1 min-h-0 flex-col overflow-hidden px-6">
        ...
        <ChatComposer
          ...
          onInputChange={(nextValue, cursorPos) => {
            speechBaseRef.current = nextValue;
            setInput(nextValue);
